import { ActorType } from './actor-type.constant';
...
export const CACHE = {
  AUTH: {
    ...
    _KEY: {
      REFRESH_TOKEN: (
        actorType: ActorType.USER | ActorType.ADMIN,
        userId: number,
        jti: string,
      ) =>
        `${PREFIX}:${GLOBAL_VER}:auth:${CACHE.AUTH._VER}:refresh:${actorType}_${userId}:jti_${jti}`,
    ...
    _PATTERN: {
      ALL_REFRESH_TOKENS: (
        ...
        `${PREFIX}:${GLOBAL_VER}:auth:${CACHE.AUTH._VER}:refresh:${actorType}_${userId}:jti_*`,
    },
  },
};
